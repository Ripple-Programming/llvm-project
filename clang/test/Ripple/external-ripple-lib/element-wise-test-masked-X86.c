...
// RUN: %clang --target=x86_64-unknown-elf -g -c -O2 -fenable-ripple -emit-llvm %S/external_library.c -o external_library_masked_x86.bc
...
void ew_multidim(size_t size, _Float16 *input, _Float16 *output) {
  ...
  size_t i;
  for (i = 0; i + BlockSizeX * BlockSizeY < size;
       i += BlockSizeX * BlockSizeY)
    output[i + BlockY * BlockSizeX + BlockX] =
        cosf16(input[i + BlockY * BlockSizeX + BlockX]);
  if (i + BlockY * BlockSizeX + BlockX < size)
    output[i + BlockY * BlockSizeX + BlockX] =
        cosf16(input[i + BlockY * BlockSizeX + BlockX]);
}
